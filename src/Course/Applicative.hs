...
instance Applicative List where
  pure ::
    a
    -> List a
  pure = (:. Nil) -- parentheses necessary for bounding the expression.
  (<*>) ::
    List (a -> b)
    -> List a
    -> List b
...
(<*) ::
  ...
  -> f a
  -> f b
(<*) = lift2 const -- sometimes, i fall into this hole where i attempt to re-write this function from scratch but then i discovered that i dont have to because i can just leverage those functions (ie.e lift2) that i just implemented.
...
replicateA ::
  ...
  -> f a
  -> f (List a)
replicateA n = sequence . replicate n -- there's a haskell function called `replicate :: Int -> a -> [a]` and its exactly what we need because we can leverage sequence which we defined earlier
