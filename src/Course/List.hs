...
product ::
  ...
  -> Int
product Nil = 1
product xs = foldRight (\a -> \b -> a * b) 1 xs
...
flatMap ::
  ...
  -> List a
  -> List b
flatMap f as = flatten $ P.fmap f as
...
seqOptional ::
  List (Optional a)
  -> Optional (List a)
seqOptional xs = case xs of
                     Nil -> Full Nil
                     (Empty :. _) -> Empty
                     ((Full a) :. t) -> myConcat (Full (a:.Nil)) (seqOptional t)

myConcat :: Optional(List a) -> Optional(List a) -> Optional(List a)
myConcat (Full xs) (Full ys) = Full ((++) xs ys)
myConcat (Full _) Empty = Empty
myConcat Empty _ = Empty
