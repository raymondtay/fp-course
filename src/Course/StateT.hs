...
instance Functor f => Functor (StateT s f) where
  (<$>) ::
    ...
    -> StateT s f a
    -> StateT s f b
  f <$> (StateT g) = StateT(\s -> (<$>)(\p -> ((,) ((f . fst) p) (snd p))) (g s))
...
instance Monad f => Applicative (StateT s f) where
  pure ::
    a
    -> StateT s f a
  pure a = StateT(\s -> return (a, s)) -- the intuition to using 'return' is of the fact that 'f ∈ Monad'
  (<*>) ::
   StateT s f (a -> b)
    -> StateT s f a
    -> StateT s f b
...
state' ::
  (s -> (a, s))
  -> State' s a
state' f = StateT(\s -> let (a', s') = f s in ExactlyOne (a', s'))
state'' f = StateT(ExactlyOne . f) -- because ExactlyOne is a functor
-- the above 2 expressions are equivalent.
...
eval' ::
  ...
  -> s
  -> a
eval' (StateT k) s = fst . runExactlyOne $ (k s)
